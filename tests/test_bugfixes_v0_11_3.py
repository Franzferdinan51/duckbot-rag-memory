...
def test_cli_doctor_flags_missing_lmstudio_reranker_model(monkeypatch):
    ...
    monkeypatch.setenv("DUCKBOT_EMBEDDING", "lmstudio")
    monkeypatch.setenv("DUCKBOT_RERANK", "1")
    monkeypatch.delenv("OPENAI_API_KEY", raising=False)
    ...
    class FakeResp:
        ...
        def json(self):
            return {"data": [{"id": "text-embedding-embeddinggemma-300m"}]}
    ...
    embed_checks = [c for c in mcp_out["checks"] if c["name"] == "LM Studio embedding model"]
    rerank_checks = [c for c in mcp_out["checks"] if c["name"] == "LM Studio reranker model"]
    assert embed_checks and embed_checks[0]["ok"] is True
    assert rerank_checks and rerank_checks[0]["ok"] is False
...
def test_skillgen_rejects_blank_name_and_description():
    """Blank skill metadata should fail fast instead of creating junk paths."""
    from src.skillgen import render_skill_md, write_skill
    with pytest.raises(ValueError, match="name is required"):
        render_skill_md(name="   ", description="ok", body_markdown="body")
    with pytest.raises(ValueError, match="description is required"):
        render_skill_md(name="Skill", description="   ", body_markdown="body")
    with pytest.raises(ValueError, match="name is required"):
        write_skill(pathlib.Path("skills"), name="   ", description="desc", body_markdown="body")


def test_brain_skill_create_trims_metadata_and_instructions(monkeypatch, tmp_path):
    """brain_skill_create should normalize whitespace before writing SKILL.md."""
    from src.mcp_server import handle_brain_skill_create
    import src.skillgen as skillgen

    captured = {}

    def fake_render_from_memory(*, name, description, instructions, example="", emoji=None):
        captured["name"] = name
        captured["description"] = description
        captured["instructions"] = instructions
        return "## When to Use\n\nUse this skill when: cleaned"

    def fake_write_skill(*, skills_dir, name, description, body_markdown, emoji=None, requires_bins=None, overwrite=False):
        captured["write_name"] = name
        captured["write_description"] = description
        captured["skills_dir"] = skills_dir
        return tmp_path / "skills" / "trimmed-skill" / "SKILL.md"

    monkeypatch.setattr(skillgen, "render_from_memory", fake_render_from_memory)
    monkeypatch.setattr(skillgen, "write_skill", fake_write_skill)

    out = _run_in_thread(handle_brain_skill_create({
        "name": "  Trimmed Skill  ",
    ...
    assert captured["name"] == "Trimmed Skill"
    assert captured["description"] == "Use this when trimming"
    assert captured["instructions"] == ["step one", "step two"]
    assert captured["write_name"] == "Trimmed Skill"
    assert captured["write_description"] == "Use this when trimming"
