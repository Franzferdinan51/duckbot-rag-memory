...
| `DUCKBOT_CHAT_MODEL` | unset | Optional chat model for LLM-assisted consolidation. Set this to the host agent's already-loaded chat model if you want `reflect()` to use one; leave it unset for regex-only consolidation with no extra model load. |
