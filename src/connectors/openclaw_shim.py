...
shared 9-tool core agent surface (`src.extensions.tools`) so the
...
Why a separate shim (not just adding verbs to hermes.py)?
  ...
  - The Hermes shim speaks the Hermes contract (sync_turn, blocks,
    ...
    contract (the 9 core tools + a generic `call <tool> '<json>'`
...
def _cmd_tools(rest: list[str]) -> Any:
    """List the 9 core tools (same surface the JSON-RPC adapter exposes)."""
