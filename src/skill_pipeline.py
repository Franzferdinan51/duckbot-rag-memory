...
def promote_candidate(
    ...
    # If the agent provided instructions_markdown, use it as the body
    # directly (after a title and When to Use section). Otherwise fall
    # back to render_from_memory's flat-instructions template.
    if instructions_markdown is not None and instructions_markdown.strip():
        # Build the body manually: title + When to Use + markdown body.
        # The trailing footer ("Auto-generated by...") is added by
        # render_skill_md in skillgen.py.
        body_lines = [
            f"## When to Use",
            "",
        ...
        if example:
            body_lines.extend(["", "## Example", "", example.strip(), ""])
        body = "\n".join(body_lines)
    else:
