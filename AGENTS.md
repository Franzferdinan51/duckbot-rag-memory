...
- MCP stdio server with 66 tools (recall/remember/forget/reflect/stats/...)
...
|   |-- mcp_server.py     # MCP stdio JSON-RPC server (66 tools)
...
- `hermes mcp add duckbot-memory --command "$(pwd)/scripts/duckbot-memory-mcp.sh"`
  (or `.bat` on Windows). 66 tools become available:
