...
- MCP stdio server with 64 tools (recall/remember/forget/reflect/stats/...)
...
|   |-- mcp_server.py     # MCP stdio JSON-RPC server (64 tools)
...
- `hermes mcp add duckbot-memory --command "$(pwd)/scripts/duckbot-memory-mcp.sh"`
  (or `.bat` on Windows). 64 tools become available:
