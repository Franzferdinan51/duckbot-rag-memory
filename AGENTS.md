...
- MCP stdio server with 68 tools (recall/remember/forget/reflect/stats/...)
...
|   |-- mcp_server.py     # MCP stdio JSON-RPC server (68 tools)
...
- `hermes mcp add duckbot-memory --command "$(pwd)/scripts/duckbot-memory-mcp.sh"`
  (or `.bat` on Windows). 68 tools become available:
